...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($matricula) || empty($modelo) || empty($fabricante) || empty($capacidad)) {
        ...
        $resultado = $crud->createAvion($matricula, $modelo, $fabricante, $capacidad, $año_fabricacion, $estado);
        if ($resultado) {
            $id_nuevo = $db->lastInsertId();
            registrar_bitacora($_SESSION['usuario_id'], $_SESSION['usuario_email'], 
                "Crear registro en tabla aviones - Matrícula: $matricula, Modelo: $modelo");
            $exito = "✅ Avión registrado exitosamente";
        } else {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        label { display: block; font-weight: 600; margin-bottom: 5px; color: #1a3a5c; }
        input, select { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; }
        input:focus, select:focus { border-color: #1a3a5c; outline: none; }
        .btn-guardar { background: #4caf50; color: white; padding: 12px 30px; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; }
        .btn-guardar:hover { background: #388e3c; }
        .btn-cancelar { background: #f44336; color: white; padding: 12px 30px; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; text-decoration: none; display: inline-block; }
        ...
        .exito { background: #d4edda; color: #155724; padding: 10px; border-radius: 5px; margin-bottom: 20px; border: 1px solid #c3e6cb; }
        .botones { display: flex; gap: 10px; }
        .info-hint { font-size: 12px; color: #6b7a8f; margin-top: 5px; }
        .matriculas-existente { background: #fff3cd; padding: 10px; border-radius: 5px; margin-bottom: 15px; font-size: 13px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="navbar">
            ...
            <a href="aviones_list.php">📋 Consultar</a>
        ...
        <div class="matriculas-existente">
            <strong>⚠️ Matrículas ya usadas:</strong> <?= implode(', ', $matriculas_existentes) ?>
