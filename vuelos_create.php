...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($numero_vuelo) || empty($origen) || empty($destino) || empty($avion_id)) {
        $error = "❌ Todos los campos obligatorios deben ser llenados";
    } else {
        $resultado = $crud->createVuelo($numero_vuelo, $avion_id, $origen, $destino, $hora_salida, $hora_llegada, $estado);
        if ($resultado) {
            $id_nuevo = $db->lastInsertId();
            registrar_bitacora($_SESSION['usuario_id'], $_SESSION['usuario_email'], 
                "Crear registro en tabla vuelos - Vuelo: $numero_vuelo, Origen: $origen, Destino: $destino");
            $exito = "✅ Vuelo programado exitosamente";
        } else {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .fila-horarios { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
...
<body>
    <div class="container">
        ...
        <div class="navbar">
            ...
            <a href="vuelos_list.php">📋 Consultar</a>
