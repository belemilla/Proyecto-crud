...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { color: #1a3a5c; border-bottom: 3px solid #1a3a5c; padding-bottom: 10px; margin-bottom: 20px; }
        .navbar { background: #1a3a5c; padding: 15px; border-radius: 8px; margin-bottom: 25px; }
        .navbar a { color: white; text-decoration: none; padding: 10px 20px; margin: 0 5px; background: rgba(255,255,255,0.2); border-radius: 5px; display: inline-block; }
        .navbar a:hover { background: rgba(255,255,255,0.3); }
        .btn-agregar { background: #4caf50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin-bottom: 20px; }
        .btn-agregar:hover { background: #388e3c; }
        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        th { background: #1a3a5c; color: white; padding: 12px; text-align: left; }
        td { padding: 12px; border-bottom: 1px solid #ddd; }
        tr:hover { background: #f5f5f5; }
        .acciones a { padding: 5px 10px; margin: 0 3px; border-radius: 3px; text-decoration: none; color: white; display: inline-block; }
        .btn-editar { background: #ff9800; }
        .btn-eliminar { background: #f44336; }
        .status-programado { background: #2196f3; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; }
        .status-en-vuelo { background: #4caf50; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; }
        .status-aterrizado { background: #ff9800; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; }
        .status-cancelado { background: #f44336; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; }
...
<body>
    <div class="container">
        ...
        <div class="navbar">
            ...
            <a href="vuelos_list.php" style="background:rgba(255,255,255,0.3);">📋 Consultar</a>
            <a href="vuelos_create.php">➕ Crear</a>
            <a href="vuelos_update.php">✏️ Modificar</a>
            <a href="vuelos_delete.php">🗑️ Eliminar</a>
            <a href="logout.php">🚪 Cerrar sesión</a>
        </div>
        <?php if (isset($_GET['mensaje'])): ?>
            <div class="mensaje mensaje-exito">✅ <?= htmlspecialchars($_GET['mensaje']) ?></div>
        <?php endif; ?>
        <?php if (isset($_GET['error'])): ?>
            <div class="mensaje mensaje-error">❌ <?= htmlspecialchars($_GET['error']) ?></div>
        <?php endif; ?>
        <a href="vuelos_create.php" class="btn-agregar">📋 Crear Nuevo Vuelo</a>
        <table>
            <thead><tr><th>ID</th><th>Vuelo</th><th>Origen</th><th>Destino</th><th>Avion</th><th>Salida</th><th>Llegada</th><th>Estado</th><th>Acciones</th></tr></thead>
            <tbody>
                <?php if (count($vuelos) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="9" style="text-align:center;padding:30px;">🚫 No hay vuelos programados</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
